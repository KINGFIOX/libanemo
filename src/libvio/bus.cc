...
io_dispatcher::io_dispatcher(std::initializer_list<io_device> device_list, size_t buffer_size):
    read_request_buffer(buffer_size), 
    write_request_buffer(buffer_size) {
...
bool io_dispatcher::request_write(uint64_t addr, width_t width, size_t req_no, uint64_t data) {
    if (req_no < write_request_buffer.firstindex()) {
        ...
        auto [cached_addr, cached_width, cached_data, cached_result] = write_request_buffer[req_no];
...
std::optional<uint64_t> mmio_agent::read(uint64_t addr, width_t width) {
    ...
    for (size_t i=old_read_count; i<read_count; ++i) {
        // In some cases, `read` and `write` can be called multiple times by the similator in a cycle
        // In this case, ignore all subsequent requests.
        // But if the same address is accessed in different ways in the same cycle.
        // It would be an error.
        if (addr == std::get<0>(buffer[i])) {
            if (width == std::get<1>(buffer[i])) {
                return std::get<2>(buffer[i]);
            } else {
                std::cerr << "libvio: Read request to the same address with different widths in the same cycle." << std::endl;
    ...
    for (size_t i=old_write_count; i<write_count; ++i) {
        ...
        if (addr == std::get<0>(buffer[i])) {
            if (width==std::get<1>(buffer[i]) && addr==std::get<2>(buffer[i])) {
                ...
                std::cerr << "libvio: Write request to the same address with different width or data in the same cycle." << std::endl;
                std::cerr << "Adderss: 0x" << std::hex << addr << std::endl;
                return false;
            }
        }
