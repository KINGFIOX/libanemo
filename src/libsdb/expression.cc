...
static const tokenizer_rule_t rules[] = {
    ...
    { std::regex("byte|half|word|sbyte|shalf|sword|pmem|vmem"), token_type_t::op, 8, 0 },
...
std::optional<std::vector<token_t>> parse_expression(const std::vector<token_t> &expr) {
    ...
    for (uint8_t prec=1; prec<max_prec; ++prec) {
        ...
        for (size_t i=0; i<expr.size(); ++i) {
            ...
            if (par_cnt < 0) {
                ...
                par_cnt==0 && // not inside any pair of parenthesis
                expr[i].type==token_type_t::op && expr[i].op.prec==prec && // is an operator of specified precedence
                i>0 && (is_numerical(expr[i-1]) || expr[i-1].type==token_type_t::parr) // is a binary operator
            ) {
                op_index = i;
            }
        }
    ...
    if (expr.front().type==token_type_t::parl && expr.size()>=3 && expr.back().type==token_type_t::parr) {
