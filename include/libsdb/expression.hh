...
struct token_t {
  ...
  union {
     ...
     * All unary operators have the same precedence (higher than binary
     * operators).
     */
    struct {
 ...
 * @return Vector of tokens in postfix notation if parsing succeeds, nullopt if
 * syntax error
 */
...
/**
 * @brief Specializes an expression by resolving variables to specific CPU
 * registers
 *
...
void specialize_expression(std::vector<token_t> &expr,
 ...
 *
 * `cpu` can be nullptr in case you are evaluating an expression not related to
 * any simulated CPU.
 *
 * @tparam WORD_T The word type, must be unsigned integer. All numbers are
 * converted to this type.
 * @param postfix_expr The expression in postfix notation (RPN)
 * @param cpu Pointer to the CPU implementation. If nullptr, the expression
 * cannot refer to CPU registers.
    ...
    case token_type_t::op:
      if (token.op.prec == 8) { // Unary operator
        ...
        if (strcmp(token.op.str, "~") == 0) {
          ...
          stack.push_back(
              libvio::zero_truncate(operand, libvio::width_t::byte));
        } else if (strcmp(token.op.str, "half") == 0) {
          stack.push_back(
              libvio::zero_truncate(operand, libvio::width_t::half));
        } else if (strcmp(token.op.str, "word") == 0) {
          stack.push_back(
              libvio::zero_truncate(operand, libvio::width_t::word));
          ...
          if (cpu != nullptr) {
            std::optional<WORD_T> val = cpu->pmem_peek(
                operand, static_cast<libvio::width_t>(sizeof(WORD_T)));
            if (val.has_value()) {
          ...
          if (cpu != nullptr) {
            std::optional<WORD_T> val = cpu->vmem_peek(
                operand, static_cast<libvio::width_t>(sizeof(WORD_T)));
            if (val.has_value()) {
              stack.push_back(val.value());
            } else {
        ...
        if (strcmp(token.op.str, "<<") == 0) {
          ...
          uint64_t shift = right & 0x3F;
          stack.push_back(
              static_cast<uint64_t>(static_cast<int64_t>(left) >> shift));
        } else if (strcmp(token.op.str, ">>>") == 0) {
          uint64_t shift = right & 0x3F;
          stack.push_back(left >> shift);
        } else if (strcmp(token.op.str, ">=") == 0) {
...
template <typename WORD_T>
std::optional<WORD_T>
evaluate_expression(const std::string &expr,
                    const libcpu::abstract_cpu<WORD_T> *cpu) {
  return evaluate_expression(parse_expression(tokenize_expression(expr))
                                 .value_or(std::vector<token_t>{}),
                             cpu);
}
