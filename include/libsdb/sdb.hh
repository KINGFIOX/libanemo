...
/**
 * @class sdb
 * @brief Template class for a simple debugger command-line interface for a
 * simulated CPU.
 * @tparam WORD_T The word type of the target CPU (e.g., uint32_t).
 */
template <typename WORD_T> class sdb {
public:
  /**
  ...
  using command_def_t = struct {
    void (*func)(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                 std::ostream &os); /**< Command function pointer */
    const char *const
        *names;       /**< Null-terminated array of command name aliases */
  ...
  static void cmd_help(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                       std::ostream &os);
  static void cmd_quit(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                       std::ostream &os);
  static void cmd_continue(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                           std::ostream &os);
  static void cmd_step(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                       std::ostream &os);
  static void cmd_status(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                         std::ostream &os);
  static void cmd_examine(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                          std::ostream &os);
  static void cmd_watch(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                        std::ostream &os);
  static void cmd_break(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                        std::ostream &os);
  static void cmd_eval(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                       std::ostream &os);
  static void cmd_trace(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                        std::ostream &os);
  static void cmd_reset(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                        std::ostream &os);

  /**
   * @var commands
   * @brief Static command definition table.
   * @details Contains all debugger commands with their handlers, aliases, and
   * help text.
   */
  static inline command_def_t commands[] = {
      ...
      {cmd_continue, (const char *const[]){"continue", "c", nullptr},
       "continue: Continue execution until breakpoint, watchpoint, or program "
       "end\n"
       "Usage:\n"
       "  continue"},
      ...
      {cmd_eval,
       (const char *const[]){"evaluate", "eval", "e", "expr", nullptr},
  ...
  libcpu::abstract_cpu<WORD_T> *cpu =
      nullptr; /**< Pointer to CPU instance being debugged */
...
template <typename WORD_T> bool sdb<WORD_T>::stopped(void) const {
  return is_stopped;
}

template <typename WORD_T>
void sdb<WORD_T>::show_command_help(const command_def_t &def,
                                    std::ostream &os) {
...
template <typename WORD_T> void sdb<WORD_T>::execute_command(std::string cmd) {
...
template <typename WORD_T> void sdb<WORD_T>::execute_command(command_t cmd) {
  if (cpu == nullptr) {
    return;
  }
  for (const command_def_t &def : commands) {
...
template <typename WORD_T>
void sdb<WORD_T>::cmd_help(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                           std::ostream &os) {
  if (args.empty()) {
...
template <typename WORD_T>
void sdb<WORD_T>::cmd_quit(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                           std::ostream &os) {
  if (!args.empty()) {
...
template <typename WORD_T>
void sdb<WORD_T>::cmd_continue(std::vector<std::string> args,
                               sdb<WORD_T> *sdb_inst, std::ostream &os) {
  if (!args.empty()) {
...
template <typename WORD_T>
void sdb<WORD_T>::cmd_step(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                           std::ostream &os) {
  size_t n;
  if (args.size() == 0) {
    n = 1;
...
template <typename WORD_T>
void sdb<WORD_T>::cmd_status(std::vector<std::string> args,
                             sdb<WORD_T> *sdb_inst, std::ostream &os) {
  if (!args.empty()) {
  ...
  os << "  pc=0x" << std::setw(word_size / 4) << std::setfill('0') << std::hex
     << sdb_inst->cpu->get_pc() << "\n";
  for (uint8_t i = 0; i < sdb_inst->cpu->n_gpr(); ++i) {
    os << std::right << std::setw(4) << std::setfill(' ')
       << sdb_inst->cpu->gpr_name(i) << "=0x" << std::hex
       << std::setw(word_size / 4) << std::setfill('0')
       << sdb_inst->cpu->get_gpr(i) << ' ';
...
template <typename WORD_T>
void sdb<WORD_T>::cmd_examine(std::vector<std::string> args,
                              sdb<WORD_T> *sdb_inst, std::ostream &os) {
  if (args.size() != 3) {
  ...
  std::optional<WORD_T> length_opt =
      evaluate_expression(args[1], sdb_inst->cpu);
  std::optional<WORD_T> word_sz_opt =
      evaluate_expression(args[2], sdb_inst->cpu);
  if (!base_opt.has_value() || !length_opt.has_value() ||
      !word_sz_opt.has_value()) {
    os << "libsdb: Invalid expression in arguments." << std::endl;
    return;
  }
  ...
  for (WORD_T addr = base; addr < base + length * word_sz; addr += word_sz) {
    ...
    auto val =
        sdb_inst->cpu->vmem_peek(addr, static_cast<libvio::width_t>(word_sz));
    if (val.has_value()) {
      os << " " << std::setfill('0') << std::setw(word_sz * 2) << std::hex
         << val.value();
    } else {
      os << " ?";
    }
...
template <typename WORD_T>
void sdb<WORD_T>::cmd_watch(std::vector<std::string> args,
                            sdb<WORD_T> *sdb_inst, std::ostream &os) {
  if (args.empty()) {
  ...
  if (args[0] == "ls") {
    ...
    os << "Watchpoint [" << sdb_inst->watchpoints.size() - 1
       << "] set: " << expr_str << " = 0x" << std::hex << val.value_or(0)
       << "\n";
  }
}

template <typename WORD_T>
void sdb<WORD_T>::cmd_break(std::vector<std::string> args,
                            sdb<WORD_T> *sdb_inst, std::ostream &os) {
  if (args.empty()) {
  ...
  if (args[0] == "ls") {
    ...
    auto it = std::find(sdb_inst->breakpoints.begin(),
                        sdb_inst->breakpoints.end(), addr.value());
...
template <typename WORD_T>
void sdb<WORD_T>::cmd_eval(std::vector<std::string> args, sdb<WORD_T> *sdb_inst,
                           std::ostream &os) {
  if (args.size() == 0) {
  ...
  os << "Octal: " << std::setw(oct_width) << std::setfill('0') << std::oct
     << val << std::endl;

  os << "Decimal: " << std::dec << val << std::endl;

  const int hex_width = sizeof(WORD_T) * 2;
  os << "Hexadecimal: " << std::setw(hex_width) << std::setfill('0') << std::hex
     << val << std::endl;
}

template <typename WORD_T>
void sdb<WORD_T>::cmd_trace(std::vector<std::string> args,
                            sdb<WORD_T> *sdb_inst, std::ostream &os) {
...
template <typename WORD_T>
void sdb<WORD_T>::cmd_reset(std::vector<std::string> args,
                            sdb<WORD_T> *sdb_inst, std::ostream &os) {
  if (args.size() == 0) {
...
bool sdb<WORD_T>::check_watchpoints(std::ostream &os) {
  for (auto &wp : watchpoints) {
    ...
    if (new_val.has_value() && wp.old_value.has_value() &&
        wp.old_value.value() != new_val.value()) {
...
template <typename WORD_T> bool sdb<WORD_T>::check_trap(std::ostream &os) {
  if (breakpoint_on_trap && cpu->get_trap().has_value()) {
    os << "Trap encountered: cause=0x" << std::hex << cpu->get_trap().value()
       << std::endl;
    return true;
  }
  return false;
...
template <typename WORD_T> const char *sdb<WORD_T>::get_prompt(void) const {
  return "sdb> ";
}
