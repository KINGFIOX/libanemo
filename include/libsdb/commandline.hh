...
struct command_t {
  ...
  /// @brief An optional pipe command that processes the output of the primary
  /// command. It is passed to `popen`. If this has a value, the output of
  /// `sdb_command` will be piped to this command.
  std::optional<std::string> pipe_command;
};

/**
 * @brief Tokenizes a command string while respecting quotes and escape
 * characters.
 *
 ...
 * @return std::optional<std::vector<std::string>> List of tokens extracted from
 * the command, nullopt if syntax error.
 */
std::optional<std::vector<std::string>> tokenize_command(std::string command);

/**
 * @brief Parses tokenized command into structured format with optional pipe
 * command.
 *
 ...
 * @return std::optional<command_t> Structured command if valid, otherwise
 * std::nullopt.
 */
...
public:
  ...
  friend popen_ostream &operator>>(std::ostream &os,
