 ...
 */
template <typename WORD_T> class sdb_difftest : public sdb<WORD_T> {
public:
  libcpu::abstract_difftest<WORD_T> *difftest =
      nullptr; /**< Differential testing interface */
...
void sdb_difftest<WORD_T>::execute_command(command_t cmd) {
  if (this->difftest == nullptr) {
    std::cerr << "libsdb: `sdb_difftest.difftest` and `sdb_difftest.cpu` "
                 "cannot be nullptr."
              << std::endl;
