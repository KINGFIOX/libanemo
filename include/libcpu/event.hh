...
inline const char *event_type_to_str(event_type_t type) {
  ...
  case event_type_t::call:
    return "call";
  case event_type_t::call_ret:
    return "call_ret";
  case event_type_t::trap:
    return "trap";
  case event_type_t::trap_ret:
    return "trap_ret";
  case event_type_t::diff_error:
    return "diff_error";
  default:
    return "unknown";
  }
 ...
 *
 * `libanemo` does not enforce strict semantics for each field. The
 * implementation treats `val1` and `val2` as generic values whose concrete
 * meanings are context-dependent. This design choice maintains simplicity,
 * flexibility, and performance, However, their interpretation must remain
 * consistent across different implementations during differential testing.
 *
 * For example:
 * - For `load` events:
 *   - When representing a load attempt that might trap: `val2` must be zero
 *   - When representing a successful load: `val2` must contain the
 * zero-extended data
 * - For `store` events:
 ...
 * some `io_dispatcher` subclasses requires `load` to represent only successful
 * loads.
 *
 * @tparam WORD_T The word type used for event values (typically uint32_t or
 * uint64_t)
 */
template <typename WORD_T> struct event_t {
  event_type_t type; ///< Type of the event
  WORD_T pc;         ///< Program counter associated with the event
  WORD_T val1; ///< Event-specific primary value (interpretation depends on
               ///< event type)
  WORD_T val2; ///< Event-specific secondary value (interpretation depends on
               ///< event type)
  ...
  bool operator==(const event_t &other) const {
    return type == other.type && pc == other.pc && val1 == other.val1 &&
           val2 == other.val2;
  }
   ...
   */
  bool operator!=(const event_t &other) const { return !(*this == other); }

  /**
   * @brief Strict weak ordering comparison for `event_t` objects.
   *
   * This operator defines a lexicographical order for events based on their
   * fields.
   *
  ...
  std::string to_string() const {
    ...
    oss << std::left << std::setw(10) << std::setfill(' ')
        << event_type_to_str(type) << " pc:0x" << std::hex
        << std::setw(sizeof(WORD_T) * 2) << std::setfill('0') << std::right
        << pc << " " << std::left << std::setw(label_width) << std::setfill(' ')
        << std::right << label1 << ":0x" << std::hex
        << std::setw(sizeof(WORD_T) * 2) << std::setfill('0') << val1 << " "
        << std::left << std::setw(label_width) << std::setfill(' ')
        << std::right << label2 << ":0x" << std::hex
        << std::setw(sizeof(WORD_T) * 2) << std::setfill('0') << val2;
