 ...
 *
 * This class provides the interface for comparing the behavior of multiple CPU
 * implementations.
 */
...
public:
  ...
  // but the number of available registers need to be limited to the smallest
  // one
  ...
  WORD_T get_pc(void) const override { return dut->get_pc(); }

  const WORD_T *get_gpr(void) const override { return dut->get_gpr(); }

  WORD_T get_gpr(uint8_t addr) const override { return dut->get_gpr(addr); }

  std::optional<WORD_T> vaddr_to_paddr(WORD_T vaddr) const override {
    return dut->vaddr_to_paddr(vaddr);
  }

  std::optional<WORD_T> vmem_peek(WORD_T addr,
                                  libvio::width_t width) const override {
    return dut->vmem_peek(addr, width);
  }

  std::optional<WORD_T> pmem_peek(WORD_T addr,
                                  libvio::width_t width) const override {
   ...
   * @note The actual content of the error is put into the event buffer as an
   * event.
   */
 ...
 * @tparam WORD_T The word type used by the CPU (e.g., uint32_t, uint64_t)
 *
 * This class implements a simple logic comparing CPU execution events for
 * differential testing. The dut can be any type of CPU, it can execute more
 * than one instructions in a cycle or execute a single instruction in multiple
 * cycles. The ref is assumed to be a single-cycle processor. The comparision is
 * based on the events logged in the event buffer. If the event buffer is a
 * nullptr, the differential testing will fail.
 */
template <typename WORD_T>
...
public:
  bool get_difftest_error(void) const override { return difftest_error; }

  void next_instruction(void) override { next_cycle(); }

  void next_cycle(void) override {
    // panic if all the event buffers are not available
    if (this->dut->event_buffer == nullptr ||
        this->ref->event_buffer == nullptr) {
      std::cerr << "libcpu: difftest error: event buffer unavaliable."
                << std::endl;
      difftest_error = true;
      return;
    }
    ...
    auto pull_events = [this](std::vector<event_t<WORD_T>> &dest,
                              event_buffer_t *buffer, size_t begin) {
      for (size_t i = begin; i < buffer->lastindex(); ++i) {
        event_t<WORD_T> event = (*buffer)[i];
        if (event.type == event_type_t::reg_write ||
            event.type == event_type_t::trap ||
            event.type == event_type_t::trap_ret) {
    ...
    dut_buffer_index =
        pull_events(dut_events, this->dut->event_buffer, dut_buffer_index);
    // step the ref
    ...
    while (ref_events.size() < dut_events.size() && !this->ref->stopped()) {
      ...
      ref_buffer_index =
          pull_events(ref_events, this->ref->event_buffer, ref_buffer_index);
    }
