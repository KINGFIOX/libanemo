 ...
 *
 * @tparam WORD_T The word type for the architecture (uint32_t for RV32,
 * uint64_t for RV64)
 *
 ...
 */
template <typename WORD_T> class privilege_module {
public:
   ...
   *
   * Performs address translation based on current privilege level and SATP
   * register. This function returns a 64-bit address regardless of WORD_T since
   * the physical address in sv32 is also wider than 32 bits.
   *
   ...
   * `op` will be populated with the `fetch` variant if successful, or `trap` if
   * not.
   *
   ...
   *
   * This function is designed for simulating processors without virtual memory.
   * `op` will be populated with the `retire` variant if successful, or the
   * `trap` variant if not.
   *
   ...
   *
   * Handles memory load operations using virtual addresses, including full
   * address translation and privilege checks according to the current memory
   * management configuration. `op` will be populated with the `retire` variant
   * if successful, or the `trap` variant if translation fails or access
   * privileges are violated.
   *
   * @param op Execution result structure containing load details
   */
   ...
   *
   * Handles memory store operations using virtual addresses, including full
   * address translation and privilege checks according to the current memory
   * management configuration. `op` will be populated with the `retire` variant
   * if successful, or the `trap` variant if translation fails or access
   * privileges are violated.
   ...
   *
   * Initializes all privilege-related registers and state to their reset
   * values.
   */
  void reset(void);
   ...
   *
   * Processes exceptions by updating CSRs and potentially changing privilege
   * level. `op` will be populated with the `retire` varient, and `op.next_pc`
   * will be changed if there is an exception to handle.
   *
   ...
   *
   * Processes interrupts by updating CSRs and potentially changing privilege
   * level. `op` will be populated with the `retire` varient, and `op.next_pc`
   * will be changed if there is an interrupt to handle.
   *
   ...
   * `op` will be populated with the `retire` varient if successful, or the
   * `trap` varient if not.
   *
...
template <typename WORD_T> void privilege_module<WORD_T>::reset(void) {
...
privilege_module<WORD_T>::vaddr_to_paddr(WORD_T vaddr) const {
...
template <typename WORD_T>
void privilege_module<WORD_T>::paddr_fetch_instruction(
...
template <typename WORD_T>
void privilege_module<WORD_T>::vaddr_fetch_instruction(
    exec_result_t &op) const {
  WORD_T vaddr = op.pc;
...
void privilege_module<WORD_T>::handle_interrupt(exec_result_t &op) {
  ...
  } else if ((sie & sip) && priv_level != priv_level_t::m &&
             (status.sie || priv_level == priv_level_t::u)) {
    for (size_t i = 0; i < word_size; ++i) {
      if (((sie & sip) >> i) & 1) {
...
#define STATUSRCS(csr_name, bit_name)                                          \
  ...
  if (write) {                                                                 \
    ...
    status.bit_name =                                                          \
        value & riscv::csr_name<WORD_T>::bit_name ? true : status.bit_name;    \
  } else if (clear) {                                                          \
    status.bit_name =                                                          \
        value & riscv::csr_name<WORD_T>::bit_name ? false : status.bit_name;   \
  }
...
void privilege_module<WORD_T>::csr_op(exec_result_t &op) {
  ...
  case riscv::csr_addr::mstatus: {
    ...
    status.mpp = status.mpp != priv_level_t::u && status.mpp != priv_level_t::s
                     ? priv_level_t::m
                     : status.mpp;
