 ...
 */
template <typename WORD_T> class abstract_cpu {
public:
  ...
  /**
   * @brief Get the program counter value of the the next instruction to be
   * comitted.
   ...
   * @note On superscalar CPUs, this function may execute more than one
   * instruction. Self-traps are counted as commited.
   */
   ...
   * @note On superscalar CPUs, this function may execute more than n
   ...
   * @note This function will not read from a MMIO address. It tiggers no
   * side-effect like caching neither. If a MMIO address is provided, `nullopt`
   * is returned.
   */
  virtual std::optional<WORD_T> vmem_peek(WORD_T addr,
   ...
   * @return Some value, whose meaning depends on implementation, if at least
   * one trap to handle, or `nullopt` if no trap.
   */
