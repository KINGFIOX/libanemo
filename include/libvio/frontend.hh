 ...
 * This class assumes that a CPU does not generate more than one MMIO request to
 * the same device in a cycle. There will be an error if there are different
 * read / write requests in the same cycle. Derived classes must implement
...
public:
   ...
   * @return Resolved I/O request structure
   */
  virtual ioreq_t resolve_write(uint64_t offset, width_t width,
